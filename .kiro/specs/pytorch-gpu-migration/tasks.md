...
- [ ] 1. Install PyTorch with CUDA support
  - [ ] 1.1 Install PyTorch for CUDA 12.x
...
- [ ] 3. Implement NNModel wrapper
  ...
  - [ ] 3.5 Implement train_with_reward()
    - Same RL-style training logic
    - Map chunk_id=-1 to target_chunk=256
    - _Requirements: 2.3_

  - [ ] 3.6 Implement train_with_supervision()
...
- [ ] 5. Update save/load for PyTorch
  - [ ] 5.1 Implement save_model()
    - File: `server/ai_engine/nn_model.py`
    - Save: `torch.save(model.state_dict(), path)`
    - Use `.pt` extension
    - _Requirements: 2.6_

  - [ ] 5.2 Implement _load_model_if_exists()
...
- [ ] 6. Port remaining methods
  - [ ] 6.1 Implement reset_weights()
    - Reinitialize model parameters
    - Backup old weights if requested
    - _Requirements: 2.6_

  - [ ] 6.2 Implement full_reset()
    - Delete saved model files
    - Rebuild fresh model
    - _Requirements: 2.6_

  - [ ] 6.3 Implement get_stats()
    - Return same dict structure
    - Include device info
    - _Requirements: 4.1_

  - [ ] 6.4 Implement get_model_summary()
    - Return string representation
    - _Requirements: 2.1_

  - [ ] 6.5 Implement _count_parameters()
...
1. [ ] PyTorch installed with CUDA support
2. [ ] `torch.cuda.is_available()` returns True
3. [ ] NNModel uses identical architecture (29→32→16→split heads)
4. [ ] All public methods have same signatures and return types
5. [ ] Entropy regularization works (loss decreases, entropy stays healthy)
6. [ ] Model saves/loads in .pt format
7. [ ] Game simulator runs end-to-end on GPU
8. [ ] Inference < 5ms, training step < 10ms on GPU
