...
export class EmblemGeometry {
    ...
    public createEmpireEmblem(): AbstractMesh {
        ...
        // Crown - Bright gold to stand out as regal headpiece
        const crownMaterial = new StandardMaterial('maskCrownMat', this.scene);
        crownMaterial.diffuseColor = new Color3(0.85, 0.65, 0.1);
        crownMaterial.specularColor = new Color3(1.0, 0.9, 0.5);
        crownMaterial.specularPower = 64;
        crownMaterial.emissiveColor = new Color3(0.2, 0.15, 0.02);
        crownMaterial.ambientColor = new Color3(0.3, 0.2, 0.05);
        ...
        // Back planks - Dark with gold trim effect
        const plankMaterial = new StandardMaterial('maskPlankMat', this.scene);
        plankMaterial.diffuseColor = new Color3(0.2, 0.15, 0.05);
        plankMaterial.specularColor = new Color3(0.5, 0.4, 0.15);
        plankMaterial.specularPower = 24;
        plankMaterial.emissiveColor = new Color3(0.03, 0.02, 0.005);
        plankMaterial.ambientColor = new Color3(0.08, 0.06, 0.02);
    ...
    public createEnergyLordsEmblem(): AbstractMesh {
        ...
        // Create main octagonal base
        const baseOctagon = this.createPolygonalBase('energyLordsBase', config);
        
        // Create energy core (central circle)
        const energyCore = this.createEnergyCore('energyLordsCore', config.size * 0.3, config.height * 1.5);
        
        // Create energy rays (star pattern)
        const energyRays = this.createEnergyRays('energyLordsRays', config.size * 0.7, config.height * 0.8, 8);
        
        // Create outer ring with energy nodes
        const outerRing = this.createEnergyNodesRing('energyLordsOuterRing', config.size * 0.9, config.height * 0.6, 8);

        // Combine all elements into single emblem
        const emblem = new Mesh('energyLordsEmblem', this.scene);
        baseOctagon.parent = emblem;
        energyCore.parent = emblem;
        energyRays.parent = emblem;
        outerRing.parent = emblem;

        // Apply Energy Lords materials with cyan/gold dual-color glow
        this.applyEnergyLordsNeonMaterials(baseOctagon, energyCore, energyRays, outerRing);

        // Add pulsing animation with energy-themed timing
    ...
    private applyEnergyLordsNeonMaterials(
        ...
        // Energy Lords colors: Cyan and Gold
        const cyanColor = new Color3(0, 1, 1); // Cyan
        const goldColor = new Color3(1, 0.8, 0); // Gold
        const cyanGlow = new Color3(0, 0.5, 0.6);
        const goldGlow = new Color3(0.5, 0.4, 0);

        // Base material - cyan foundation
        const baseMaterial = this.materialManager.createCustomMaterial(
        ...
        // Core material - bright gold energy center
        const coreMaterial = this.materialManager.createCustomMaterial(
            'energyLordsEmblemCore',
            goldColor,
            {
                emissive: new Color3(goldGlow.r * 1.5, goldGlow.g * 1.5, goldGlow.b * 1.5),
                specular: new Color3(1, 0.8, 0.2),
                roughness: 0.1
            }
        );
        core.material = coreMaterial;

        // Rays material - gold energy beams
        this.applyMaterialToChildren(rays, 'energyLordsEmblemRay', goldColor, goldGlow);

        // Nodes material - cyan energy points
        this.applyMaterialToChildren(nodes, 'energyLordsEmblemNode', cyanColor, cyanGlow);
    }
